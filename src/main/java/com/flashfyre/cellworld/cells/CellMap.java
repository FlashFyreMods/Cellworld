...
public record CellMap(List<Integer> layerScales, CellSelectionSet cells) {
    ...
    public static final Codec<CellMap> DIRECT_CODEC = RecordCodecBuilder.create(
            inst -> inst.group(
                    ...
                    CellSelectionSet.CODEC.fieldOf("cell_selector").forGetter(cellMap -> cellMap.cells)
            ).apply(inst, CellMap::new)
    );

    public static final Codec<Holder<CellMap>> CODEC = RegistryFileCodec.create(CellworldRegistries.CELL_MAP_REGISTRY_KEY, DIRECT_CODEC);
    ...
    public Cell getCell(int x, int z) {
        ...
        Either<Holder<Cell>, CellSelectionSet> current = Either.right(this.cells);
        int cellIndex = nucleiPositions.size()-1;
        RandomSource r = new LegacyRandomSource(0);
        while (current.right().isPresent()) {
            CellSelectionSet currentCells = current.right().orElseThrow();
    ...
    public static void bootstrap(BootstrapContext<CellMap> ctx) {
        ...
        ctx.register(NETHER, new CellMap(List.of(64, 32, 16), new WeightedRandomSelectionSet(weightedCellEntries.getOrThrow(WeightedCell.NETHER))));
    }
