...
public record CellEntry(Either<Cell, CellSelector> value) {
    ...
    public static final Codec<CellEntry> CODEC = RecordCodecBuilder.create(instance -> instance.group(
            Codec.either(Cell.CODEC, CellSelector.CODEC).fieldOf("value").forGetter(CellEntry::value)
    ).apply(instance, CellEntry::new)
    );
    ...
    public CellEntry(CellSelector cells) {
